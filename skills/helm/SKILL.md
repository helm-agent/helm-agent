...
- `helm daemon status --json` → `{ health, stats }`. `health` is the `/health` body (`{ pid, daemonId, uptime, version }`); `stats` is the raw `DebugStatsResponse` (`{ memory: { rssBytes, heapUsedBytes, totalRssBytes, descendantCount }, cpuPercent, threadCount, uptimeMs, sampledAt }`) on success, or `null` when the daemon is unreachable or `/debug/stats` failed. `cpuPercent` is % of one core; the first sample after a daemon (re)start reports `0` (no baseline yet).
