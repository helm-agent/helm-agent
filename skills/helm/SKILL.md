...
Background tasks: while a turn runs, the agent can spawn background Bash commands and subagents (the SDK's task stream). There is no `helm session` CLI for these — they are driven from the UI (session-chat cards) via two daemon routes that mirror `POST /sessions/:id/cancel`. `POST /sessions/:id/tasks/:taskId/stop` stops a running task (the SDK's kill); `POST /sessions/:id/tasks/background` with optional body `{ toolUseId? }` sends in-flight foreground task(s) to the background (the SDK's Ctrl+B — omit `toolUseId` to background all, pass a tool_use_id to target one). Both require an active run (else `409 no_active_run`) and return `202` — `{ accepted: true }` for stop, `{ accepted: true, backgrounded: boolean }` for background (`backgrounded: false` means a targeted `toolUseId` matched no foreground task, a benign race). `stopTask` keys on the SDK `taskId`; `backgroundTasks` keys on the tool_use_id. See `docs/designs/2026-05-29-background-task-controls.md`.
