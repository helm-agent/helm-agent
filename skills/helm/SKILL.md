...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `daemon`     | The background process itself. Speaks HTTPS by default with the shared CA at `~/.helm/tls/ca.pem`; BYO via `--tls-cert <path>` `--tls-key <path>` on `start`. Auto-leaf SANs cover the bind host's IPs; `--hostname <name>` (repeatable) on `start`/`restart` adds DNS/IP SANs so a `--bind 0.0.0.0` daemon is pairable via a domain / Tailscale MagicDNS name (not just its IP). `--bind` / `--tls-cert` / `--tls-key` / `--hostname` are persisted to `~/.helm/daemon/start-args.json` and replayed by `helm daemon restart` / `helm update`. The Electron desktop app's embedded daemon opts back to plain HTTP via `HELM_DAEMON_TLS=0` set by the supervisor — no operator action needed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `start`, `stop`, `restart`, `status`, `logs`, `reset`, `pair`, `tokens`                                                                      |
| `server`     | The optional `helm server` (HTTPS proxy + pairing). Default bind: `127.0.0.1:8220` with random-port fallback if 8220 is taken. Override with `--bind <host>:<port>`. Speaks HTTP/2 over TLS to bypass Chrome's 6-per-origin H1 socket cap. Auto-mints a leaf via `mkcert` if installed, otherwise self-signed; trust anchor at `~/.helm/tls/ca.pem`. Auto-leaf SANs include the bind host (so `--bind <tailscale-ip>:<port>` "just works"); on first non-loopback bind the selfsigned anchor rotates once to extend SANs (TOFU-pinned clients must re-run `server add`). **Domain/hostname access:** `--hostname <name>` (repeatable) and `HELM_SERVER_HOSTNAMES` (comma-separated) add DNS/IP SANs to the cert AND allow the `https://<name>:<port>` Origin, decoupled from the bind — so a domain / Tailscale MagicDNS name works while the listener stays on an IP (`--bind 0.0.0.0:<port>`); each name gets its own printed `web: https://<name>:<port>/#code=…` login URL. BYO via `--tls-cert <path>` `--tls-key <path>` (e.g. a `tailscale cert` file) for warning-free HTTPS. `start` backgrounds by default; pass `--foreground` to attach. `restart` preserves the bearer token for ~5 min. **Persistent start args:** `--bind` / `--tls-cert` / `--tls-key` / `--hostname` are persisted to `~/.helm-server/start-args.json` on successful start and replayed by `helm update`, a no-arg `helm server restart`, and the `helm restart` umbrella (explicit flags win over the persisted values). A loopback bind combined with `--hostname` prints a stderr warning — the name is unreachable until you also `--bind 0.0.0.0:<port>`. **`add` accepts `--ca <path>`** to override the default TOFU peer-chain harvest with an operator-supplied CA; without `--ca`, the SHA-256 fingerprint of the pinned chain is printed to stderr for out-of-band verification. **`ca`** prints the self-signed CA (PEM to stdout, or `--path` for `~/.helm/tls/ca.pem`) so a client can trust it — see "Trusting the Helm CA" below. | `start`, `stop`, `restart`, `status`, `stats [<id>]`, `add [--ca <path>]`, `ca [--path]`, `ls`, `rm`                                         |
| `start`      | Top-level umbrella: bring up daemon + server (both detached). No subcommands. Idempotent — re-running is safe and short-circuits already-running processes. `--tls-cert` / `--tls-key` are forwarded to **server only**. `--hostname` is forwarded to the server AND (when `--bind` makes the daemon reachable) to the **daemon cert** as an extra SAN, so a `--bind 0.0.0.0` daemon can be paired via that domain / Tailscale MagicDNS name (not just its IP). `--bind <host>[:<port>]` binds **both** daemon + server to `<host>`: the daemon always gets an OS-assigned port (so it's reachable for cross-server pairing), and the server uses `<port>` if given (else an OS-assigned port). The umbrella never uses `<port>` for the daemon — to pin the daemon's own port, use `helm daemon start --bind <host>:<port>`. `--foreground` is rejected with a hint; run `helm daemon start --foreground` / `helm server start --foreground` in separate shells if you need attached lifecycles.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | (no subcommands)                                                                                                                             |
| `session`    | Live agent conversations                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `new`, `send`, `tail`, `ls`, `show`, `cancel`, `approve`, `attach`, `branch`, `rewind`, `export`, `search`, `rename`, `archive`, `pin`, `rm` |
...
| `terminals`  | The persistent **pty-host** that owns interactive terminal shells so they survive daemon/server restarts + upgrades. Spawned lazily by the daemon on the first terminal; one per `HELM_HOME`, independent of the daemon lifecycle. `status` prints running/not-running + live terminal count; `kill-all` is the ONLY way to intentionally end terminals (a plain `helm daemon stop`/`restart` leaves them alive) and also stops the host. Survives version upgrades via a versioned IPC handshake; a host older than the daemon's minimum makes spawns fail `pty_host_incompatible` ("restart terminals to upgrade" → run `kill-all`). The internal `host` subcommand is daemon-spawned only, not for direct use. See `docs/designs/2026-07-05-pty-host-survive-restart.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `status`, `kill-all`                                                                                                                         |
...
Background tasks: while a turn runs, the agent can spawn background Bash commands and subagents (the SDK's task stream); a `run_in_background` Bash is born backgrounded. There is no `helm session` CLI for these — they are driven from the UI (session-chat cards + the footer pill) via daemon routes that mirror `POST /sessions/:id/cancel`. `POST /sessions/:id/tasks/:taskId/stop` stops a running task (the SDK's kill); `POST /sessions/:id/tasks/background` with optional body `{ toolUseId? }` sends in-flight foreground task(s) to the background (the SDK's Ctrl+B — omit `toolUseId` to background all, pass a tool_use_id to target one). `tasks/background` requires an active run (else `409 no_active_run`); `tasks/:taskId/stop` does NOT (a background task survives the turn and must be stoppable between turns). Both return `202` — `{ accepted: true }` for stop, `{ accepted: true, backgrounded: boolean }` for background (`backgrounded: false` means a targeted `toolUseId` matched no foreground task, a benign race). `stopTask` keys on the SDK `taskId`; `backgroundTasks` keys on the tool_use_id. `GET /sessions/:id/tasks` (no active-run requirement) returns `{ tasks: TaskState[] }` — the daemon's in-memory live background-task registry (born-backgrounded Bash + Ctrl+B-flipped subagents, keyed by `taskId`), the liveness authority the UI seeds the footer pill from on mount/reload; it survives across turns for the daemon-process lifetime but is empty after a daemon restart (no SDK list-tasks API). See `docs/designs/2026-05-29-background-task-controls.md` and `docs/designs/2026-05-29-cross-turn-live-background-task-surface.md`.
...
- `helm session branch <id> [--at <messageUuid>]` — branch into a new sessionId. Use for cross-tool clean state.
...
When unsure between branch and rewind: **branch** when you want a clean policy/state slate or you intend to keep both branches; **rewind** when you just want to undo recent turns on the same session.

### Reaching `helm server` by IP or domain

The server binds `127.0.0.1:8220` by default (loopback only). To reach it from another device:

- **By IP** (e.g. a Tailscale `100.x` address): `helm server start --bind 0.0.0.0:8220`. The auto cert already covers every non-internal NIC and the origin gate accepts each bind IP, so `https://<ip>:8220/` works after you accept/trust the cert.
- **By domain / MagicDNS name:** add `--hostname <name>` (repeatable) or `HELM_SERVER_HOSTNAMES=<a,b>` — decoupled from the bind. This adds the name as a cert SAN, allows its `https://<name>:<port>` Origin, and prints a `web: https://<name>:<port>/#code=…` login URL. Open THAT URL (each origin bootstraps its own per-origin token).
...
```bash
helm server ca > helm-ca.pem      # PEM to stdout
helm server ca --path             # → ~/.helm/tls/ca.pem
```

- **macOS:** `sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain helm-ca.pem` (or drag into Keychain Access → System → set "Always Trust").
- **iOS:** AirDrop/email `helm-ca.pem`, install the profile (Settings → General → VPN & Device Management), then Settings → General → About → Certificate Trust Settings → enable full trust.
- **Windows:** `certutil -addstore -f Root helm-ca.pem` (elevated).
- **Firefox:** it uses its own store — Settings → Privacy & Security → Certificates → View Certificates → Authorities → Import.

Zero-warning alternative (no CA install): supply a real cert with `--tls-cert`/`--tls-key`, e.g. Tailscale's `tailscale cert <name>` output or a Let's Encrypt pair.
...
- `helm daemon status` prints the existing `daemon running  pid=… port=… id=… uptime=…s version=…` line. When the daemon also answers the authed `/debug/stats` probe, the same line is enriched with `cpu=<n.n>%  rss=<n>MB  totalRss=<n>MB  threads=<n|—>  descendants=<n>` (threads is `—` on platforms where it can't be sampled). If `/debug/stats` is unavailable (older daemon, transient `ps` error, timeout) the command degrades to the health-only line and still exits 0.
- `helm daemon status` (human output) has two distinct **not-running** messages, both exit `1`: `daemon not running (stale state for pid=X) — run 'helm daemon start' to recover` when a state file exists but its recorded pid is dead (the recoverable, stale-state case — `start` self-heals it), versus `daemon state found but unreachable (pid=…, port=…)` when the pid is alive but `/health` does not answer (a hung daemon — restart needed). Don't conflate a stale-state exit `1` with a hung daemon. When no state file exists at all it prints `daemon not running` (also exit `1`).
- `helm daemon status --json` → `{ health, stats }`. `health` is the `/health` body (`{ pid, daemonId, uptime, version }`); `stats` is the raw `DebugStatsResponse` (`{ memory: { rssBytes, heapUsedBytes, totalRssBytes, descendantCount }, cpuPercent, threadCount, uptimeMs, sampledAt }`) on success, or `null` when the daemon is unreachable or `/debug/stats` failed. The `--json` envelope shape is the same (`{ health: null, stats: null }`) for both the dead-pid and hung-daemon cases — the stale-vs-hung distinction is surfaced only in the human output above. `cpuPercent` is % of one core; the first sample after a daemon (re)start reports `0` (no baseline yet).
- `helm server stats [<id>] [--json]` reads those same metrics for a **remote/registered** daemon through the running `helm server` proxy. With `<id>`: one daemon (exit 0 on success; exit 1 on unknown id, daemon unreachable, or other error). With no id: one line per daemon from the manual registry + auto-discovery, best-effort — exits 0 as long as the server itself answered, marking unreachable daemons inline. `--json` emits a single `{ id, ok, stats?, error? }` envelope for `<id>`, or an array of them for batch mode. Requires `helm server start` to be running; otherwise prints a "helm-server not running" hint and exits 1.
...
4. If a session is wedged: `helm session cancel <id>`, then `helm session tail <id>` to confirm it returned to idle. If state is bad, `helm session branch <id>` to escape.
